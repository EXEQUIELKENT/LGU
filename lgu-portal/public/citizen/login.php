...
$authConfigFile = __DIR__ . '/../../includes/config/auth_config.php';
...
function sendUnlockEmail(mysqli $conn, string $email, string $firstName): bool {
    ...
    try {
        ...
        if ($isDomain) {
            $protocol = 'https';
            $unlockUrl = $protocol . '://' . $host . '/lgu-portal/public/citizen/login.php?unlock_token=' . $unlockToken;
        } else {
...
if (isset($_POST['forgot_password_submit'])) {
    ...
    if ($result->num_rows === 1) {
        ...
        try {
            ...
            if ($isDomain) {
                $protocol = 'https';
                $resetUrl = $protocol . '://' . $host . '/lgu-portal/public/citizen/login.php?reset_token=' . $resetToken;
            } else {
                $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
