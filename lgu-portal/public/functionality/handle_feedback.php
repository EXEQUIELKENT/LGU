...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: ../citizen/citizen_feedback.php');
    exit;
}
...
if (isset($_FILES['photos']) && is_array($_FILES['photos']['name'])) {
    $upload_dir_rel = 'uploads/feedback/';
    ...
    foreach ($_FILES['photos']['name'] as $i => $fname) {
        ...
        $fname_gen = 'fbk_' . $feedback_id . '_' . $i . '_' . time() . '.' . strtolower($ext);
        $safe     = $upload_dir . $fname_gen;
        $safe_rel = $upload_dir_rel . $fname_gen;
        if (move_uploaded_file($tmp, $safe)) {
            $img_stmt->bind_param('is', $feedback_id, $safe_rel);
            $img_stmt->execute();
        }
    }
    $img_stmt->close();
...
header('Location: ../citizen/citizen_feedback.php?submitted=1');
exit;