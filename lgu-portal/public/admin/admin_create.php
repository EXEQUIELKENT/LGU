...
if (!$isAdmin) {
    session_unset();
    session_destroy();
    header("Location: " . cimm_url('../citizen/login.php'));
    exit;
}
                        ...
                        if (!$pendingInsert->execute()) {
                            ...
                            try {
                                    ...
                                    <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:14px;padding:44px 36px;box-shadow:0 4px 20px rgba(0,0,0,0.1)">
                                        ...
                                        <div style="background:linear-gradient(135deg,#f0fff4,#e6f9ee);
                                            ...
                                            <div style="font-size:11.5px;color:#888;margin-top:8px;word-break:break-all;">
                                                <a href="https://cimm.infragovservices.com/lgu-portal/public/citizen/citizencimm.php?staff=infrastructure_staff_2026_qr8p"
                                                   style="color:#219653;">https://cimm.infragovservices.com/lgu-portal/public/citizen/citizencimm.php?staff=infrastructure_staff_2026_qr8p</a>
                                            </div>
                                        </div>
                                ...
                                $mail->AltBody = "LGU Portal - Email Verification\n\n" .
                                    ...
                                    "https://cimm.infragovservices.com/lgu-portal/public/citizen/citizencimm.php?staff=infrastructure_staff_2026_qr8p\n\n" .
                                    "© " . date('Y') . " LGU Portal";
...
@media (max-width: 768px) {
    ...
    /* ! BUG FIX — on mobile, .sidebar-profile-btn is position:relative (in normal
       flow, see rule above) instead of desktop's position:absolute, and
       .mobile-dark-mode-btn (position:absolute, effective top ~60px) replaces
       the desktop .sidebar-toggle. The site-logo card's margin/padding were
       still tuned for the desktop absolute-positioning layout, so on mobile the
       card only clipped the very bottom edge of both buttons instead of framing
       them. Pulled the card up to start above both buttons' tops and pushed its
       top padding down so the logo image still clears their bottom edges. */
...
<div class="sidebar-nav" id="sidebarNav">
    ...
    <div class="sidebar-top">
        ...
        <ul class="nav-list">
            ...
            <li>
                <a href="<?= cimm_url_attr('user_management.php') ?>" class="nav-link" data-tooltip="User Management">
