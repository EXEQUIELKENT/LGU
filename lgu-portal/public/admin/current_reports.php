...
// ── Track which admin gave feedback (returned the report to engineer) ──────────
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'admin_return_report') {
        ...
        $stmt = $conn->prepare(
            "UPDATE request_resolutions rr
             JOIN   reports r ON r.res_id = rr.res_id
             SET    rr.status = 'Approved', rr.admin_return_note = ?, rr.highlight_fields = ?, rr.admin_feedback_by = ?
             WHERE  r.rep_id  = ?
               AND  rr.status = 'Pending Admin Approval'"
        );
        if (!$stmt) {
        ...
        $stmt->bind_param("ssii", $returnNote, $highlightFields, $adminFeedbackById, $repId);
        $ok = $stmt->execute(); $err = $stmt->error; $aff = $stmt->affected_rows; $stmt->close();
        while (ob_get_level() > 0) ob_end_clean();
        if (!$ok)    { echo json_encode(['success' => false, 'message' => $err]); exit; }
        if ($aff < 1){ echo json_encode(['success' => false, 'message' => 'Report is not in Pending Admin Approval state.']); exit; }
    ...
    if ($action === 'log_engineer_profile_view') {
        ...
        if ($repId > 0 && $engId > 0) {
            ...
            log_report_activity($conn, 'current_reports', $repId, 'engineer_profile_viewed',
...
foreach ($rows as $row) {
    ...
    if (!empty($row['evidence_images']))
        $imgs = array_map(fn($p) => '../' . $p, array_values(array_filter(explode(',', $row['evidence_images']))));
    $rowsJson[] = [
...
.gis-dp-trigger-btn {
    width: 100%; padding: 5px 9px; border: 1.5px solid var(--border-color, rgba(0,0,0,.1));
    border-radius: 7px; font-size: 12px; color: var(--text-secondary);
    background: var(--bg-primary, #f8fafc); font-family: inherit; cursor: pointer;
...
.gis-dp-overlay {
    ...
    background: var(--bg-secondary, #fff); border: 1.5px solid rgba(55,98,200,.22);
...
.gis-dp-footer { display: flex; align-items: center; justify-content: flex-end; gap: 6px; margin-top: 4px; border-top: 1px solid var(--border-color, rgba(0,0,0,.1)); padding-top: 8px; }
...
.gis-dp-clear-btn { background: none; border-color: var(--border-color, rgba(0,0,0,.1)); color: var(--text-secondary); }
...
@media (max-width: 768px) {
    ...
    .report-card .road-monitoring-badge { font-size: 12px; padding: 1px 6px; }
...
/* ── View button — soft outline style (matches case_management.php's
   ...
   at rest, solid fill on hover. Current Reports keeps its orange accent. ── */
.btn-view-rep {
    display:inline-flex; align-items:center; gap:5px;
    background:rgba(230,81,0,.08); color:#e65100;
...
.btn-view-rep:hover {
    ...
    transform:translateY(-1px); box-shadow:0 4px 14px rgba(230,81,0,.35);
}
.btn-view-rep:active { transform:translateY(0); }
[data-theme="dark"] .btn-view-rep { background:rgba(255,152,0,.14); color:#ffb74d; border-color:rgba(255,152,0,.4); }
[data-theme="dark"] .btn-view-rep:hover { background:#e65100; color:#fff; border-color:#e65100; }
    ...
    <div class="desktop-nav-inner">
        ...
        <div class="nav-actions">
            ...
            <button class="nav-btn notif-btn" id="notifBtn" title="Notifications">
                ...
                </span><span class="notif-badge hidden" id="notifBadge"></span>
            </button>
        </div>
    </div>
</div>
        ...
        <ul class="nav-list">
            ...
            <li class="nav-dropdown-item open">
                ...
                <ul class="nav-sub-list">
                    <li><a href="#" class="nav-link nav-sub-link active"><i class="fas fa-spinner"></i><span>Current Reports</span></a></li>
...
<?php endif; ?>
    ...
    <div class="he-no-district-banner">
        ...
        <div>
            ...
            <span>Set your district in your <a href="<?= cimm_url_attr('profile.php') ?>#aeDistrictSection">profile</a> to view and manage reports in your area.</span>
        </div>
    </div>
    <?php endif; ?>
                ...
                <div class="gis-dd-wrap" id="reportsTriageMapPeriodWrap">
                    ...
                    <div class="gis-dd-menu" id="reportsTriageMapPeriodMenu">
                        ...
                        <div class="gis-dd-picker">
                            ...
                            <button type="button" class="gis-dp-trigger-btn" id="reportsTriageMapPickMonthBtn">
                                <i class="fas fa-calendar"></i>
                                <span id="reportsTriageMapPickMonthLabel">Click to select month</span>
                            </button>
                            <input type="hidden" id="reportsTriageMapPickMonth">
                        </div>
                        <div class="gis-dd-picker">
                            <label><i class="fas fa-calendar-check"></i> Specific Day</label>
                            <button type="button" class="gis-dp-trigger-btn" id="reportsTriageMapPickDayBtn">
                                <i class="fas fa-calendar-check"></i>
                                <span id="reportsTriageMapPickDayLabel">Click to select date</span>
                            </button>
                            <input type="hidden" id="reportsTriageMapPickDay">
                        </div>
                    </div>
                </div>
...
async function loadEngineers() {
    ...
    try {
        ...
        let url = '../functionality/get_engineers.php';
...
function renderPortalList(engineers, query, infrastructure, district) {
    ...
    if (dist) {
        ...
        distMatched   = engineers.filter(e => (e.district || '').trim().toLowerCase() === distLower);
        distUnmatched = engineers.filter(e => (e.district || '').trim().toLowerCase() !== distLower);
    }
...
async function doAssignEngineer(repId, engineerId, engineerName, engData) {
    ...
    try {
        const res  = await fetch('../functionality/assign_engineer.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    document.querySelectorAll(`.engineer-cell[data-rep-id="${repId}"]`).forEach(cell => {
        if (CAN_ASSIGN_ENGINEER && engineerId) {
            cell.innerHTML = `<span class="eng-name-with-profile">` +
                `<button class="eng-profile-btn" onclick="openEngineerProfileById(${parseInt(engineerId)}, ${parseInt(repId)})" title="View Engineer Profile">${btnInner}</button>` +
...
function rdpInit(overlayId, displayId, hiddenId, gridId,
    ...
    function openPicker() {
        ...
        // displayEl on scroll — reposition on every scroll/resize while open.
        window.addEventListener('scroll', onRdpScroll, true);
        window.addEventListener('resize', onRdpScroll);
    }
    function onRdpScroll() {
    ...
    function closePicker() {
        ...
        window.removeEventListener('scroll', onRdpScroll, true);
        window.removeEventListener('resize', onRdpScroll);
    }
...
(function () {
    ...
    setupDropdown(periodWrap, periodBtn, periodMenu, periodLabelEl, PERIOD_LABELS, 'all', (val) => {
        ...
        if (window._reportsTriageMapDpReset) window._reportsTriageMapDpReset();
        applyMapSearch();
    });
    ...
    (function () {
        const monthBtn   = document.getElementById('reportsTriageMapPickMonthBtn');
        const dayBtn     = document.getElementById('reportsTriageMapPickDayBtn');
        if (!monthBtn || !dayBtn) return;
        const monthHidden = document.getElementById('reportsTriageMapPickMonth');
        const dayHidden    = document.getElementById('reportsTriageMapPickDay');
        const monthLabelEl = document.getElementById('reportsTriageMapPickMonthLabel');
        const dayLabelEl   = document.getElementById('reportsTriageMapPickDayLabel');

        const overlay = document.createElement('div');
        overlay.className = 'gis-dp-overlay';
        overlay.innerHTML = [
            '<div class="gis-dp-nav-row">',
              '<button class="gis-dp-nav-btn" id="reportsTriageMapDpPrev" type="button">&#8592;</button>',
              '<div class="gis-dp-nav-center">',
                '<button class="gis-dp-month-lbl" id="reportsTriageMapDpMonthLbl" type="button"></button>',
                '<button class="gis-dp-year-lbl"  id="reportsTriageMapDpYearLbl"  type="button"></button>',
              '</div>',
              '<button class="gis-dp-nav-btn" id="reportsTriageMapDpNext" type="button">&#8594;</button>',
            '</div>',
            '<div class="gis-dp-year-grid"   id="reportsTriageMapDpYearGrid"></div>',
            '<div class="gis-dp-month-grid"  id="reportsTriageMapDpMonthGrid"></div>',
            '<div class="gis-dp-weekdays" id="reportsTriageMapDpWeekdays">',
              '<span>Su</span><span>Mo</span><span>Tu</span><span>We</span><span>Th</span><span>Fr</span><span>Sa</span>',
            '</div>',
            '<div class="gis-dp-day-grid"    id="reportsTriageMapDpDayGrid"></div>',
            '<div class="gis-dp-footer">',
              '<button class="gis-dp-clear-btn" id="reportsTriageMapDpClear" type="button">Clear</button>',
              '<button class="gis-dp-done-btn"  id="reportsTriageMapDpDone"  type="button">Done</button>',
            '</div>'
        ].join('');
        ...
        const prevBtn   = document.getElementById('reportsTriageMapDpPrev');
        const nextBtn   = document.getElementById('reportsTriageMapDpNext');
        const monthLbl  = document.getElementById('reportsTriageMapDpMonthLbl');
        const yearLbl   = document.getElementById('reportsTriageMapDpYearLbl');
        const yearGrid  = document.getElementById('reportsTriageMapDpYearGrid');
        const monthGrid = document.getElementById('reportsTriageMapDpMonthGrid');
        const weekdays  = document.getElementById('reportsTriageMapDpWeekdays');
        const dayGrid   = document.getElementById('reportsTriageMapDpDayGrid');
        const clearBtn  = document.getElementById('reportsTriageMapDpClear');
        const doneBtn   = document.getElementById('reportsTriageMapDpDone');
        ...
        window._reportsTriageMapDpReset = function () {
